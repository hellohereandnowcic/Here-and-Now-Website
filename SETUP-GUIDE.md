...
6. Now open the **Here & Now Website** folder on your computer (the one
   ...
   *everything* inside it — `index.html`, `content.json`, all the other
   `content-*.json` files, the `admin` folder, the logo and photo image
   files, all of it — and drag the whole selection into the browser
   window, onto the area that says "drag files here."
...
3. You should see a simple screen with a list of sections down the side —
   Homepage, Our Story, Looking Back, Shipwright Ensemble, Team, Mailing
   List — each one full of plain text boxes and photo-upload buttons.
...
- The mailing-list signup form still points at a placeholder web address.
  Open `index.html`, find the comment that says `BREVO FORM`, and follow
  the instructions right there to swap in your real Brevo sign-up link.
- If you get hold of the real **Noe** font files, drop them into a
  `fonts` folder in the repository (see the comment near the top of
  `index.html`) — until then, headings use a similar-looking fallback.
